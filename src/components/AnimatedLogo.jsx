    ...
    <svg className="absolute inset-0 w-full h-full" viewBox="-30 -30 60 60">
      {Array.from({ length: NUM_ORBITS }).map((_, i) => (
        <motion.circle
          key={i}
          r={3 - i * 0.6}
          fill={i === 0 ? '#10b981' : i === 1 ? '#14b8a6' : '#ec4899'}
          cx={0}
          cy={-r}
          initial={{ opacity: 0 }}
          animate={{
...
function UnderlineDraw({ size }) {
  return (
    <motion.div
      ...
      style={{
        background: 'linear-gradient(90deg, #10b981, #14b8a6, #ec4899, #14b8a6, #10b981)',
        backgroundSize: '200% 100%',
      }}
    ...
    return (
      ...
      >
        <div className={`relative ${iconSize} flex-shrink-0`}>
          ...
          />
          <motion.div
            className="absolute inset-0 rounded-full bg-teal-500 blur-lg"
            animate={{ opacity: [0, 0.4, 0], scale: [0.8, 1.1, 0.8] }}
            transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
          />
          <svg viewBox="0 0 48 46" className="relative w-full h-full drop-shadow-[0_0_6px_rgba(16,185,129,0.4)]">
            <motion.path
              d={K_PATH}
              fill="url(#k-gradient)"
              animate={{ filter: ['drop-shadow(0 0 2px rgba(16,185,129,0.3))', 'drop-shadow(0 0 8px rgba(16,185,129,0.6))', 'drop-shadow(0 0 2px rgba(16,185,129,0.3))'] }}
  ...
  return (
    <div className="relative flex flex-col items-center gap-2">
      ...
      >
        {/* Expanding ring */}
        <motion.div
          className="absolute -inset-8 rounded-full border-2 border-emerald-500/40"
          variants={ringVariants}
        />
        <motion.div
          className="absolute -inset-12 rounded-full border border-teal-500/20"
          variants={ringVariants}
          custom={1}
        />
        ...
        <motion.div className={`relative ${iconSize} flex-shrink-0`} variants={iconVariants}>
          ...
          />
          <motion.div
            className="absolute inset-0 rounded-full bg-teal-500 blur-xl"
          ...
          {/* K path */}
          <svg viewBox="0 0 48 46" className="relative w-full h-full z-10 drop-shadow-[0_0_10px_rgba(16,185,129,0.5)]">
            <motion.path
              d={K_PATH}
        ...
        {/* Cursor */}
        <motion.span
          className={`${cursorSize} text-emerald-400 font-light`}
