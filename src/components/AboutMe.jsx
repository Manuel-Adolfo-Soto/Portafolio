    ...
    <section id="sobre-mi" className="relative py-16 px-4 overflow-hidden">
      ...
      >
        ...
        <motion.div variants={item} className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {metrics.map((m) => (
            <div key={m.label} className="glass rounded-xl p-5 text-center card-hover group">
              ...
              <div className="text-slate-900 dark:text-white font-medium text-sm">{m.label}</div>
              <div className="text-slate-400 dark:text-slate-500 text-xs mt-0.5">{m.sub}</div>
            </div>
          ))}
        </motion.div>
          ...
          <motion.div variants={item} className="md:col-span-3 space-y-4">
            {bio.map((p, i) => (
              <div key={i} className="glass rounded-xl p-6 card-hover">
                <p className="text-slate-600 dark:text-slate-300 leading-relaxed">{p}</p>
              </div>
            ))}
          </motion.div>
        ...
        <motion.div variants={item} className="mt-8 grid md:grid-cols-2 gap-6">
          <div className="glass rounded-xl p-6 card-hover">
            <h3 className="text-slate-900 dark:text-white font-semibold mb-4">{t('about.methodology.title')}</h3>
            <div className="space-y-3">
              {methodology.map((item) => (
                <div key={item.label} className="flex items-start gap-3">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-2 shrink-0" />
                  <div>
                    <p className="text-slate-900 dark:text-white text-sm font-medium">{item.label}</p>
          ...
          <div className="glass rounded-xl p-6 card-hover">
            <h3 className="text-slate-900 dark:text-white font-semibold mb-4">{t('about.interests.title')}</h3>
