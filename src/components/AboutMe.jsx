...
export default function AboutMe() {
  ...
  return (
    <section id="sobre-mi" className="relative py-16 px-4 overflow-hidden">
      ...
      >
        ...
        <motion.div variants={item} className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {metrics.map((m) => (
            <div key={m.label} className="glass rounded-xl p-5 text-center card-hover group">
              ...
              <div className="text-gray-900 dark:text-white font-medium text-sm">{m.label}</div>
              <div className="text-gray-400 dark:text-gray-500 text-xs mt-0.5">{m.sub}</div>
            </div>
          ))}
        </motion.div>
          ...
          <motion.div variants={item} className="md:col-span-3 space-y-4">
            {bio.map((p, i) => (
              <div key={i} className="glass rounded-xl p-6 card-hover">
                <p className="text-gray-600 dark:text-gray-300 leading-relaxed">{p}</p>
              </div>
            ))}
          </motion.div>
              ...
              <div className="space-y-0">
                {highlights.map((h, i) => (
                  <div key={i} className="relative pl-8 pb-6 last:pb-0 group">
                    ...
                    <div className="absolute left-0 top-1 w-5 h-5 rounded-full bg-gray-100 dark:bg-gray-900 border-2 border-emerald-500 flex items-center justify-center group-hover:border-teal-500 transition-colors z-10 text-[10px]">
                      {h.icon}
                    </div>
                    <div>
                      <span className="text-xs font-bold text-emerald-400 font-mono">{h.year}</span>
                      <h4 className="text-gray-900 dark:text-white text-sm font-medium mt-0.5">{h.label}</h4>
                      <p className="text-gray-400 dark:text-gray-500 text-xs mt-0.5">{h.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <motion.div variants={item} className="mt-8 grid md:grid-cols-2 gap-6">
          <div className="glass rounded-xl p-6 card-hover">
            <h3 className="text-gray-900 dark:text-white font-semibold mb-4">{t('about.methodology.title')}</h3>
          ...
          <div className="glass rounded-xl p-6 card-hover">
            <h3 className="text-gray-900 dark:text-white font-semibold mb-4">{t('about.interests.title')}</h3>
            <div className="flex flex-wrap gap-2">
              {interests.map((interest) => (
                <span
                  key={interest}
                  className="px-3 py-1.5 text-sm bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-full border border-gray-200 dark:border-gray-700 hover:border-emerald-500/50 hover:text-emerald-400 transition-all"
                >
                  {interest}
                </span>
              ))}
