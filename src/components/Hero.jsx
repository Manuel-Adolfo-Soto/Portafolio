    ...
    >
      ...
      >
        ...
        <motion.div variants={itemVariants} className="mb-6 flex flex-wrap justify-center gap-4">
          ...
          <span className="inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-medium text-violet-400 bg-violet-500/10 rounded-full border border-violet-500/30 backdrop-blur-sm">
        ...
        {/* Description */}
        <motion.p variants={itemVariants} className="text-base sm:text-lg text-slate-500 dark:text-slate-400 max-w-2xl mx-auto mb-12 leading-relaxed">
          {t('hero.description')}
        </motion.p>
        ...
        <motion.div variants={itemVariants} className="mt-16">
          ...
          <div className="flex justify-center gap-6 flex-wrap">
            {['Angular', 'NestJS', 'TypeScript', 'PostgreSQL', 'Docker'].map((tech) => (
              <span key={tech} className="text-sm text-slate-400 dark:text-slate-500 hover:text-cyan-400 transition-colors font-mono">
                {tech}
              </span>
            ))}
          </div>
